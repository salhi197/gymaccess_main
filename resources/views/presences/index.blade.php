...
@section('content')
                    <div class="card">
                            <div class="card-body table1">
                                      ...
                                      <form method="post" action="{{route('presence.filter2')}}">
                                      @csrf
                                      <div class="row">
                                          <div class="col-md-3">
                                ...
                                <table id="example1"  class="table table-striped table-bordered no-wrap">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$presence->id ?? ''}}</td>
                                                <td>
                                                  {{$presence->getMembre()['nom'] ?? ''}}
                                                  {{$presence->getMembre()['prenom'] ?? ''}}
                                                  {{$presence->nom_prenom ?? ''}}
...
{{$presence->created_at->format('H')+2 ?? ''}}:{{$presence->created_at->format('i:s') ?? ''}}
                                                    </span>
                                                  </h3>
                                                </td>
                                                <td class="display-4">
                                                        <a class="btn bubbly-button" href="{{route('presence.destroy',['presence'=>$presence->id])}}" 
